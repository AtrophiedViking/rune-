...
void opaqueFrameBuffersCreate(State* state)
{
    ...
    for (uint32_t i = 0; i < frameBufferCount; i++) {
        ...
        if (state->config->msaaSamples == VK_SAMPLE_COUNT_1_BIT) {
            // No MSAA: color = sceneColor, depth = singleDepth
            std::array<VkImageView, 2> attachments = {
                state->texture->sceneColorImageView,   // color (offscreen)
                state->texture->singleDepthImageView   // depth (single-sample)
            };
            ...
            PANIC(vkCreateFramebuffer(state->context->device,
                ...
                "Failed To Create Opaque Framebuffer (no MSAA)");
        }
        else {
            // MSAA: color = msaaColor, depth = msaaDepth, resolve = sceneColor
            std::array<VkImageView, 3> attachments = {
                state->texture->colorImageView,        // MSAA color
                state->texture->msaaDepthImageView,    // MSAA depth
                state->texture->sceneColorImageView    // resolve color
            };
            ...
            PANIC(vkCreateFramebuffer(state->context->device,
                ...
                nullptr,
                &state->buffers->opaqueFramebuffers[i]),
                "Failed To Create Opaque Framebuffer (MSAA)");
        }
    }
}
...
void transparentFrameBuffersCreate(State* state)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        PANIC(vkCreateFramebuffer(state->context->device, &fb, nullptr,
