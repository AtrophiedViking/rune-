...
VkFormat findDepthFormat(State* state) {
    return findSupportedFormat(state, { VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT }, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
}
...
void imageCreate(State* state, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, uint32_t mipLevels, VkSampleCountFlagBits numSamples) {
    ...
    assert(state->context->device != VK_NULL_HANDLE);
    ...
    printf("imageCreate: %ux%u format=%d usage=0x%x tiling=%d samples=%d mipLevels=%u\n",
       width, height, format, usage, tiling, numSamples, mipLevels);
...
{
    if (mipLevels == 0) {
        std::cout << "transitionImageLayout: mipLevels == 0, "
            << "image=" << image
            << " oldLayout=" << oldLayout
            << " newLayout=" << newLayout
            << std::endl;
    ...
    VkCommandBuffer commandBuffer =
        beginSingleTimeCommands(state, state->renderer->commandPool);
    ...
    // ---------------------------------------------------------
    // FIXED: aspectMask is chosen based on FORMAT, not layout
    // ---------------------------------------------------------
    bool isDepth =
